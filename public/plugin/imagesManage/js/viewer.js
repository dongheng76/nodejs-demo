...
(function(factory) {
	...
	function Viewer(element, options) {
		...
		if (this.options.customEvent != null) {
			EVENT_CUSTOM = this.options.customEvent + "." + NAMESPACE;
		}
		this.isImg = false;
		this.isBuilt = false;
		this.isShown = false;
	...
	Viewer.prototype = {
	    ...
	    init : function() {
		    ...
		    if (options.inline) {
			    ...
			    $this.on(EVENT_CUSTOM || EVENT_CLICK, $.proxy(this.start, this));
		    }
	    },
	    ...
	    resize : function() {
		    ...
		    if (this.isPlayed) {
			    this.$player.find(SELECTOR_IMG).one(EVENT_LOAD, $.proxy(this.loadImage, this)).trigger(EVENT_LOAD);
		    }
	    },
	    ...
	    mousedown : function(event) {
		    ...
		    if (action) {
			    ...
			    // IE8 has `event.pageX/Y`, but not
				// `event.originalEvent.pageX/Y`
			    // IE10 has `event.originalEvent.pageX/Y`, but not
				// `event.pageX/Y`
	    ...
	    view : function(index) {
		    ...
		    if (!this.isShown || this.isPlayed || index < 0 || index >= this.length || this.isViewed && index === this.index) {
			    return;
		    }
	    ...
	    zoomTo : function(ratio, hasTooltip, _event, _zoomable) {
		    ...
		    if (isNumber(ratio) && this.isViewed && !this.isPlayed && (_zoomable || options.zoomable)) {
			    ...
			    if (_event && (originalEvent = _event.originalEvent)) {
				    ...
				    image.left -= (newWidth - width) * (((center.pageX - offset.left) - image.left) / width);
				    image.top -= (newHeight - height) * (((center.pageY - offset.top) - image.top) / height);
			    } else {
	    ...
	    destroy : function() {
		    ...
		    if (this.options.inline) {
			    ...
			    $this.off(EVENT_CUSTOM || EVENT_CLICK, this.start);
		    }
	    ...
	    requestFullscreen : function() {
		    ...
		    if (this.isFulled && !document.fullscreenElement && !document.mozFullScreenElement && !document.webkitFullscreenElement && !document.msFullscreenElement) {
	    ...
	    change : function(event) {
		    ...
		    case 'zoom':
			    this.zoom(function(x1, y1, x2, y2) {
				    ...
				    return (z2 - z1) / z1;
			    }(abs(this.startX - this.startX2), abs(this.startY - this.startY2), abs(this.endX - this.endX2), abs(this.endY - this.endY2)), false, event);
	...
	Viewer.DEFAULTS = {
	    ...
	    // Show the tooltip with image ratio (percentage) when zoom in or zoom
		// out
	    tooltip : true,
	    ...
	    // Type: String (an image attribute) or Function (should return an image
		// URL)
	    url : 'src',

	    // custom definition trigger viewer event
	    customEvent : null,
	...
	Viewer.TEMPLATE = ('<div class="viewer-container">' + '<div class="viewer-canvas"></div>' + '<div class="viewer-footer">' + '<div class="viewer-title"></div>' + '<ul class="viewer-toolbar">' + '<li class="viewer-zoom-in" data-action="zoom-in"></li>' + '<li class="viewer-zoom-out" data-action="zoom-out"></li>' + '<li class="viewer-one-to-one" data-action="one-to-one"></li>' + '<li class="viewer-reset" data-action="reset"></li>' + '<li class="viewer-prev" data-action="prev"></li>' + '<li class="viewer-play" data-action="play"></li>' + '<li class="viewer-next" data-action="next"></li>' + '<li class="viewer-rotate-left" data-action="rotate-left"></li>' + '<li class="viewer-rotate-right" data-action="rotate-right"></li>' + '<li class="viewer-flip-horizontal" data-action="flip-horizontal"></li>' + '<li class="viewer-flip-vertical" data-action="flip-vertical"></li>' + '</ul>' + '<div class="viewer-navbar">' + '<ul class="viewer-list"></ul>' + '</div>' + '</div>'
	    + '<div class="viewer-tooltip"></div>' + '<div class="viewer-button" data-action="mix"></div>' + '<div class="viewer-player"></div>' + '</div>');
